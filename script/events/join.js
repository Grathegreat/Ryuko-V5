    ...
    const checkban = data.banOut || []
        ...
        api.changeNickname(`${botname} ai`, threadID, botID);
                ...
                try {
                        ...
                        let { threadName, participantIDs, adminIDs } = await api.getThreadInfo(threadID);

                        const threadData = global.data.threadData.get(parseInt(threadID)) || {};
                        const antiJoin = threadData.antiJoin || false;
                        ...
                        for (id in event.logMessageData.addedParticipants) {
                                ...
                                if (antiJoin && adminIDs.some(admin => admin.id == botID)) {
                                        api.removeUserFromGroup(userID, threadID);
                                        return api.sendMessage(`🚫 ANTI-JOIN ACTIVATED!\n\n${userName} was removed from the group!\nNo new members allowed! 😤`, threadID);
                                }
                        }
